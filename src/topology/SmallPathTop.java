...
public class SmallPathTop {
	private Vertex<DistancWeight>[] vertexs = new Vertex[1000000];
...
//        Random ran = new Random();
	...
	public void generateSmallPathToOther(Vertex<DistancWeight> topVertex) {
		if (topVertex != null) {
			...
			topVertex.setInWeight(new DistancWeight(0));
			LinkedList<Vertex<DistancWeight>> vertexQueue = new LinkedList<Vertex<DistancWeight>>();
			vertexQueue.add(topVertex);
			
			while (!vertexQueue.isEmpty()) {
				
				Vertex<DistancWeight> vertPoll = vertexQueue.poll();
				vertPoll.setInQueue(false);

				List<Vertex<DistancWeight>> vetexs = vertPoll.getAdjacents();
				for (Vertex<DistancWeight> vert : vetexs) {

					DistancWeight nextWei = vertPoll.getNextWeight(vert);
